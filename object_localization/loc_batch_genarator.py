...
class BatchGenerator(keras.utils.Sequence):
    def __init__(self, path, list_ids, labels,  batch_size=32, 
                 img_height=256, img_width=256, n_channels=3 ,
                 #n_classes=2, 
                 shuffle=True):
    ...
    def __data_generation(self, list_ids_batch):
        ...
        y = np.empty([self.batch_size,4], dtype = np.int)
        ...
        for i, ID in enumerate(list_ids_batch):
            ...
            # get original img size to Compute relative y 
            origin_img = Image.open(var)
            origin_width, origin_height =origin_img.size
            
            X[i,] = np.array(self.img_resize(var, self.img_height, self.img_width))
                        
            # Store class
            ID_LAB=ID.replace('jpg', 'xmls')
            y[i] = (int(self.labels[ID_LAB]['xmin'])/origin_width)*self.img_width, \
                    (int(self.labels[ID_LAB]['ymin'])/origin_height)*self.img_height, \
                    (int(self.labels[ID_LAB]['xmax'])/origin_width)*self.img_width, \
                    (int(self.labels[ID_LAB]['ymax'])/origin_height)*self.img_height \
