...
class Transformer(tf.keras.Model):
  def __init__(self, vocab_size, word_emb_dim, nb_layers, nb_heads, seq_max_length, dropout, mask=True):
    ...
    self.decoder = Decoder(vocab_size, word_emb_dim, nb_layers, nb_heads, seq_max_length, mask)
    self.final_layer = tf.keras.layers.Dense(vocab_size, name='output_dense_layer')
    
    

  def call(self, enc_sequence, dec_sequence):

    enc_output = self.encoder(enc_sequence)
    dec_output = self.decoder(dec_sequence, enc_output)
